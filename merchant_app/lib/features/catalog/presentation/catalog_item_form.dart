...
import '../../../platform/merchant_money.dart';
...
/// Add/edit form for one catalog item (issue #133), shown in a modal bottom
/// sheet from [CatalogScreen]. Fields shown depend on [vertical] -- see
/// `merchant_vertical.dart` / `merchant_catalog_item.dart` for which
/// columns each vertical's item table actually has.
...
class _CatalogItemFormState extends State<CatalogItemForm> {
  ...
  Future<void> _submit() async {
    ...
    final priceCents = MerchantMoney.parseToCents(_priceController.text);
    if (priceCents == null) {
      setState(() {});
      return;
    }
    ...
    final draft = MerchantCatalogItem(
      ...
      imageUrl: widget.vertical.itemSupportsImage
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                ...
                validator: (value) =>
                    MerchantMoney.parseToCents(value ?? '') == null
                    ? 'Enter a valid, non-negative price.'
                    : null,
              ),
              if (vertical.itemSupportsImage) ...[
                const SizedBox(height: 12),
                TextFormField(
