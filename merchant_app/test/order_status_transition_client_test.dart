...
/// Issue #135's third acceptance bullet, filling a real gap left by #133/#134:
/// those issues' own test suites (`merchant_orders_controller_test.dart`,
/// `order_detail_screen_test.dart`, `orders_screen_test.dart`) all test the
/// *error-handling path* -- what happens when something (the fake standing in
/// for the RPC) rejects a status change -- by handing the controller a status
/// value the real UI would never produce (e.g. `confirmed -> out_for_delivery`
/// directly). None of them assert the converse: that the client's own call
/// sites (`OrderDetailScreen`'s "advance"/"cancel" buttons, which decide their
/// target status via `nextForwardOrderStatus`/`canCancelOrder`) never even
/// *attempt* an illegal transition in the first place. That is what this file
/// tests, as defense in depth -- not a substitute for the server-side check in
/// `is_legal_order_status_transition` (issue #131), which remains the real
/// enforcement point.
///
...
/// `FakeMerchantOrdersRepository` in `fakes/fake_merchant_repositories.dart`,
