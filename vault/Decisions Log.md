...
Owner decisions: (1) **delivery address is not required anywhere** — checkout asks only for an optional delivery note; name + phone come from the profile server-side (RPCs fill blanks from `profiles`); the Addresses screen stays in code but is unlinked. (2) **No payments right now** — Wallet screen/route removed; checkout shows a static "Pay on delivery" line (orders still record `cash_on_delivery`). (3) **Fresh Meat and Electronics reuse the grocery engine** (a `grocery_stores.store_type` column, one store list per type), not new verticals. **Revised same day:** each of Grocery / Fresh Meat / Electronics has its **own cart** (and own store pages, checkout, routes) and its **own look** (red / blue palettes, "Fresh Meat cart" titles) — one `GroceryController` per `GroceryStoreType`, still one engine underneath. (4) Cart and checkout are one shared screen each across verticals. (5) **"Order again"** replaces the cart after a "Replace your cart?" prompt, at today's prices, skipping unavailable items. (6) Remaining rewards copy removed.
...
### 2026-09-24 — Profile editing lives in Settings only; standalone Edit Profile screen removed
Owner dislikes the separate Edit Profile screen: it and the Profile tab's edit button are deleted, and all profile edits happen from Settings → Account. Pattern chosen after reviewing the mobile standard (Uber Eats/DoorDash/system settings): **tap a row → bottom sheet editing just that field** (not an inline always-editable form). Same validation as registration (name/phone/DOB can change but not be cleared). **Email is editable too**, via Supabase's confirm-new-address flow.

### 2026-09-23 — Registration DOB/phone fields: no age gate, no OTP, split name fields
Adding required First name / Last name / Phone / DOB to `RegisterScreen` (see [[Status Log]] 2026-09-23 part 2), owner made three explicit calls when asked: (1) DOB has no minimum-age enforcement — collected but any age is accepted; (2) phone is captured as plain validated text, no OTP/SMS verification step; (3) name stays two fields (first/last), matching the existing `profiles.firstname`/`lastname` columns, not a single "full name" field. Revisit only if a specific vertical (e.g. pharmacy/alcohol) later needs real age-gating or phone trust becomes load-bearing for something (e.g. delivery-driver contact).

### 2026-09-13 — Mass decision session clearing every long-blocked `waiting-on-you` issue at once
In one sitting the owner answered every open board-worker question and relabeled the issues back to `todo`: **#3** profiles columns are `firstname`/`lastname` live (schema.sql was stale). **#8** currency standardizes on integer smallest-units (cents) going forward, confirmed `menu_items.price` is live decimal dollars needing a real `*100` migration, not just a naming fix. **#9** the rewards feature is removed entirely, not built for real or marked "coming soon". **#12** the fake payment-methods screen is removed (not wired up), since #30 makes payment cash-on-delivery-only. **#16** native identifier is `com.zivo.app` (RunnerTests: `com.zivo.app.RunnerTests`), same across platforms. **#30** payment approach for launch is cash-on-delivery only — no processor integration, but add `payment_method`/`payment_status` columns now so a future processor doesn't need another migration. **#40** crash reporting is Firebase Crashlytics, but land the SDK-independent global-error-handling half first since Firebase project credentials aren't provisioned yet. **#74** approved #34 (core-tables migration) so the RLS-enable fix can follow it, per #74's own suggested sequencing. **#78** shared platform layer is a `delivery_addresses` table referenced by FK from each vertical's order table (not a full `orders` supertype), clean-break rewrite since no real orders exist yet. **#132** merchant app is same-repo, package id `com.zivo.merchant` — but still explicitly **not** a thing the board worker may pick up unattended (needs a human to actually scope/kick off a new top-level Flutter app), a carve-out the owner reaffirmed in the same comment that answered everything else. All 6 issues that were unambiguously actionable (#3/#8/#9/#12/#16/#30) were implemented and merged the same run — see [[Status Log]] 2026-09-13.
