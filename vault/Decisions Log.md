...
**Mechanical limitation discovered while implementing**: this routine (`trig_017jPchk8L4LVskUZMGwiDDG`) was created via the web/API directly, not by any agent's `create_trigger` call — `update_trigger`/`delete_trigger` both refuse to touch a routine they didn't create, and a routine can only disable itself from its own fired session. No agent session (this one included) can edit or delete it. The user pastes prompt updates into the routine's own web UI instead; this vault stays the source of truth for what the prompt *should* say, verified against what's actually live only when someone checks the UI directly.

### 2026-08-29 — Backlog PR/conflict merging kept OUT of the scheduled board worker; separate manual-only routine created instead

Same-day follow-up to the self-merge decision above: user initially asked for the board worker to also reconcile already-open PRs (the backlog + known conflict pairs), not just PRs it opens fresh. First implementation attempt folded this into the 5-hourly cron routine's own loop — user rejected that ("no is not a good idea") before it was pushed. Rationale inferred from the rejection plus the request that followed: an autonomous cron routine silently sweeping and merging a growing backlog of older PRs every 5 hours, with no per-run visibility into *which* backlog PRs it touched, is a bigger blast radius than self-merging freshly-implemented, freshly-tested work — better kept as something the user explicitly kicks off when they actually want it done, not something that happens in the background on a timer.

**Decision**: reverted the cron-routine changes (discarded the uncommitted edits to `Multi-Agent Setup.md`/`Conventions.md`/`Status Log.md` describing backlog-sweeping as part of the board worker; `trig_017jPchk8L4LVskUZMGwiDDG`'s prompt was never actually updated to include it, since the user hadn't pasted the earlier draft in yet). Created a **second, separate routine** instead — `trig_01BhBGebMrc2tJfHy1MyHT6R`, "eatzy PR merger (manual)" — with no `cron_expression`/`run_once_at` at all (poke-only), so it only ever runs when the user explicitly fires it. Same job (DoD-check, resolve conflicts against `master`, merge or explain), same repo, same DoD/branch conventions, deliberately kept as a distinct trigger rather than a mode flag on the existing one, so the two are independently visible/manageable (the scheduled one for new work, the manual one for backlog cleanup) and neither's cadence/prompt edits can accidentally leak into the other.

**Mechanical gap hit and worked around**: `create_trigger` (fresh-session-per-fire mode) has no `source_url`/repo-binding parameter — a first attempt (`trig_014XYuGXqKxoPJGvpSndKUNv`) came back with an explicit warning that its fired sessions would have no `mcp__github__*` (or any MCP) tools at all, confirmed by comparing its `job_config` (no `sources` field) against the working board-worker's `job_config` (has `sources: [{git_repository: ...}]`). `list_connectors` confirmed GitHub isn't a claude.ai-style connector here either (empty result for "github") — repo access on this platform comes from a session's own `source_url` binding, not the connector-grant mechanism `create_trigger`'s `connectors` param controls. Fix: deleted the broken trigger, created a dedicated persistent session (`session_01QyXoT2fRz27JGbgjRq9ZQN`) via `create_session` with `source_url` set to this repo (mirroring how any interactive session gets repo access), then created the real trigger bound to that session via `persistent_session_id` instead of fresh-session-per-fire. Tradeoff: that session's conversation history accumulates a little across repeated manual fires, since `persistent_session_id` mode resumes the same session each time rather than spawning a clean one — accepted, since no mechanism currently exists to get both a clean per-fire session and real repo access through `create_trigger` alone. See [[Multi-Agent Setup]]'s "Manual PR merger routine" section for the full spec.
