...
- `lib/features/` — `auth`, `cart`, `checkout`, `home`, `onboarding`, `orders`, `profile`, `restaurant`, `settings`, `super_app`, `support` (`rewards` and `wallet` were removed — owner decisions 2026-09-13 and 2026-09-25; there are no payments, orders are pay-on-delivery). Cart and checkout are one shared `CartView`/`CheckoutView` in `lib/widgets/` used by every vertical (2026-09-25).
...
- `lib/services/` — the "vertical" domains: `food`, `grocery`, `pharmacy` (`cleaning` was removed entirely in issue #50, 2026-08-15 — don't reintroduce it). Fresh Meat and Electronics run on the **grocery engine** (filtered by `grocery_stores.store_type`) but each has its own `GroceryController` instance (own cart), routes and palette — see `GroceryStoreType` (2026-09-25)
