...
| 79 | Order status can never change after creation — no UPDATE path exists | 2026-08-27 (11th run) | **Human replied 2026-08-30** (no bot footer) — resolved via the new #128/#131 merchant self-service decision, not directly here. Left `waiting-on-you` deliberately: #131 (not yet merged) does the actual implementation; this issue just tracks "order status can change" until #131 lands, then close it |
...
#16/#40 still have only the agent's own clarifying-question comment as of 2026-08-26 — no human reply on either yet (going on ~15 and ~13 days respectively). #78 still no human reply. #79/#74 are new as of the 13th run (2026-08-30), too early to expect a reply on #74 yet.
...
| 128 | Merchant self-service (tracking) | Umbrella issue for the new #129-135 chain — decided directly with the human 2026-08-30, see its body for the full shape/decisions. Implement children in order: #129 (schema) → #130 (RLS) → #131 (RPCs, also resolves #79) → #132 (**flagged in its own body as needing human scoping — no subagent owns creating a new top-level Flutter app**) → #133/#134 (merchant app screens) → #135 (QA) |

## Remaining `todo`, not yet picked up

**As of the 13th run (2026-08-30)**: #129-135 (merchant self-service chain, oldest-first #129 next), #123 (food_home_screen wrapper indirection, approved this batch), #137-144 (9 new small UI/bug issues from a fresh audit pass — cart/snackbar/icon consistency, category icon stretch, home screen spacing, checkout inline errors, pharmacy/grocery per-store ordering, plus #136 "[Blocking] place_food_order reintroduced pre-address 2-arg overload" — note #136 is a real regression bug, not just style, worth prioritizing for the next run's slots despite FIFO normally putting #123 first). Re-check via `list_issues` before assuming this list is complete — a lot landed in one batch this run and more may have been added since.
