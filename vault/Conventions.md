...
- **New finding 2026-09-18 (79th-ish run, issue #236): this run's own session permission system (the "auto mode" classifier) outright denied an `Agent` dispatch whose prompt instructed the dispatched agent to self-merge, tagged `[Merge Without Review]` — this fired at dispatch time, before any merge tool call was even attempted, and gave no path to override it (not a per-action confirmation prompt, a hard denial with no live human available to approve it). Had to redispatch the same task with the self-merge instruction removed; the agent implemented, verified, and opened the PR (#237) but left it open for human review instead. **Not yet known whether this is a one-off environment/session quirk or a real policy change** — the routine's own prompt still says to self-merge, and this note's own policy above (2026-08-27) still describes self-merging as normal. Future runs: try the self-merge dispatch as usual; if it's denied the same way, leave the PR open, note it in the PR body (as #237 does), and flag it here/in Status Log again so a pattern can be confirmed before assuming the 2026-08-27 policy is dead.
